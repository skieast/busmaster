...
/* API STUBS */
int32_t WINAPI CanInitDriver(char* options){ return 0; }
void WINAPI CanDownDriver(void){ return; }
int32_t WINAPI CanSetOptions(char* options){ return 0; }
int32_t WINAPI CanDeviceOpen(uint32_t  index, char* parameter){ return 0; }
int32_t WINAPI CanDeviceClose(uint32_t  index){ return 0; }
int32_t WINAPI CanApplaySettings(uint32_t  index){ return 0; }

int32_t WINAPI CanSetMode(uint32_t  index, unsigned char can_op_mode, uint16_t  can_command){ return 0; }
int32_t WINAPI CanSet(uint32_t  index, uint16_t  obj_index, uint16_t  obj_sub_index, void* data, int32_t size){ return 0; }
int32_t WINAPI CanGet(uint32_t  index, uint16_t  obj_index, uint16_t  obj_sub_index, void* data, int32_t size){ return 0; }

int32_t WINAPI CanTransmit(uint32_t  index, struct TCanMsg* msg, int32_t count){ return 0; }
void WINAPI CanTransmitClear(uint32_t  index){ return; }
uint32_t  WINAPI CanTransmitGetCount(uint32_t  index){ return 0; }
int32_t WINAPI CanTransmitSet(uint32_t  index, uint16_t  cmd,
	uint32_t  time){ return 0; }
int32_t WINAPI CanReceive(uint32_t  index, struct TCanMsg* msg, int32_t count){ return 0; }
void WINAPI CanReceiveClear(uint32_t  index){ return; }
uint32_t  WINAPI CanReceiveGetCount(uint32_t  index){ return 0; }

int32_t WINAPI CanSetSpeed(uint32_t  index, uint16_t  speed){ return 0; }
int32_t WINAPI CanSetSpeedUser(uint32_t  index, uint32_t  value){ return 0; }
char* WINAPI CanDrvInfo(void){ return 0; }
char* WINAPI CanDrvHwInfo(uint32_t  index){ return 0; }
int32_t WINAPI CanSetFilter(uint32_t  index, struct TMsgFilter* msg_filter){ return 0; }

int32_t WINAPI CanGetDeviceStatus(uint32_t  index, struct TDeviceStatus* status){ return 0; }

void WINAPI CanSetPnPEventCallback(void(__stdcall* event)(uint32_t  index, int32_t status)){ return; }
void WINAPI CanSetStatusEventCallback(void(__stdcall* event)
	(uint32_t  index, struct TDeviceStatus* device_status)){ return; }
void WINAPI CanSetRxEventCallback(void(__stdcall* event)
	(uint32_t  index, struct TCanMsg* msg, int32_t count)){ return; }

void WINAPI CanSetEvents(uint16_t  events){ return; }
uint32_t  WINAPI CanEventStatus(void){ return 0; }
/* END API STUBS */
